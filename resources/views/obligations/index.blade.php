            ...
            <div class="col-12 col-sm-6 col-lg-4">
                ...
                @if(auth()->user()->isAdmin())
                    <div style="display:flex; gap:0.5rem;">
                        <a href="{{ route('obligations.edit', $obligation) }}" class="btn btn-sm btn-warning flex-fill">✏️ تعديل</a>
                        <form method="POST" action="{{ route('obligations.destroy', $obligation) }}" class="flex-fill">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-sm btn-danger w-100" onclick="return confirm('متأكد من حذف هالالتزام؟')">🗑️ حذف</button>
                        </form>
                    </div>
                @endif
            </div>
