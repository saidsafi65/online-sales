        ...
        <div class="card" style="border-radius: 20px; padding: 2rem; box-shadow: 0 10px 25px rgba(0,0,0,0.1);">
            <form method="POST" action="{{ route('users.update', $user) }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="password" name="password" autocomplete="new-password" class="form-control @error('password') is-invalid @enderror" placeholder="أدخل كلمة مرور جديدة" style="border-radius: 10px; border: 2px solid #e2e8f0; padding: 0.875rem;">
                    @error('password')<span class="invalid-feedback">{{ $message }}</span>@enderror
                </div>

                <div class="mb-3">
                    <label class="form-label" style="font-weight: 600; margin-bottom: 0.75rem;">تأكيد كلمة المرور</label>
                    <input type="password" name="password_confirmation" autocomplete="new-password" class="form-control" placeholder="أعد إدخال كلمة المرور" style="border-radius: 10px; border: 2px solid #e2e8f0; padding: 0.875rem;">
                </div>
