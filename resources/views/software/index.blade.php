...
:root {
    --brand: {{ app()->bound('currentTenant') ? (app('currentTenant')->brand_primary_color ?? '#dc2626') : '#dc2626' }}; --brand-light: color-mix(in srgb, var(--brand) 15%, white); --brand-dark: {{ app()->bound('currentTenant') ? (app('currentTenant')->brand_accent_color ?? '#991b1b') : '#991b1b' }};
...
@media (min-width: 992px) {
    .shop-layout { display:grid;grid-template-columns:250px 1fr;gap:1.75rem;align-items:start; }
}
.shop-header { margin-bottom:1.75rem;padding:2rem 2.25rem;border-radius:var(--r-xl);background:linear-gradient(135deg, var(--brand-dark) 0%, var(--brand) 100%);color:white;position:relative;overflow:hidden; }
...
.contact-pill { background:rgba(255,255,255,.16);border:1px solid rgba(255,255,255,.28);border-radius:50px;padding:.5rem 1.2rem;font-size:.86rem;font-weight:700;backdrop-filter:blur(6px);display:flex;align-items:center;gap:.5rem;color:white;text-decoration:none;transition:background var(--t); }
...
.card-img-placeholder { width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:2.75rem;color:var(--slate-300);background:linear-gradient(135deg,var(--slate-100) 0%,var(--brand-light) 100%); }
...
#modal-img-col { position:relative;background:var(--slate-100);height:100%;min-height:0;overflow:hidden;display:flex;align-items:center;justify-content:center; }
...
#modal-img-placeholder { display:none;width:100%;height:100%;align-items:center;justify-content:center;font-size:5rem;color:var(--slate-300);background:linear-gradient(135deg,var(--slate-100) 0%,var(--brand-light) 100%); }
...
#modal-category { font-size:.73rem;font-weight:800;color:var(--brand);text-transform:uppercase;letter-spacing:1px;flex-shrink:0; }
#modal-name { font-size:1.4rem;font-weight:900;color:var(--slate-900);margin:.2rem 0 0; line-height:1.25;flex-shrink:0; }
#modal-dev { font-size:.85rem;color:var(--slate-500);flex-shrink:0; }
...
@media (max-width:1024px) { .product-grid { grid-template-columns:repeat(auto-fill,minmax(195px,1fr)); } }
...
@push('mobile-filter-bar')
    @php
        $__mfbActiveCount = collect(request()->only(['search','category','in_stock']))
    ...
    <form method="GET" action="{{ route('software.index') }}" id="filterForm" autocomplete="off">
        <div class="shop-layout">
            ...
            <main class="main-col">
                <div class="toolbar">
                    @php
                        $activeFilterCount = collect(request()->only(['search','category','in_stock']))
                            ->filter(fn($v) => $v !== null && $v !== '')->count();
                    @endphp
                    ...
                    <div class="search-wrap">
                        <input type="text" name="search" id="searchInput" value="{{ request('search') }}" placeholder="ابحث بالاسم أو الشركة المطورة أو التصنيف..."
                ...
                <div id="resultsContainer">
                    @include('software.partials.results')
                </div>
            </main>
        </div>
    </form>
