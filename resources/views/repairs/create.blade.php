...
@section('title', request('type') === 'software' ? 'إضافة تركيب ويندوز/برامج' : 'إضافة صيانة جديدة')

@section('content')
<div class="container py-4">
    <div class="card">
    <div class="card-header"><h3 class="mb-0">{{ request('type') === 'software' ? 'إضافة تركيب ويندوز/برامج' : 'إضافة صيانة جديدة' }}</h3></div>
    <div class="card-body">
            @if ($errors->any())
            ...
            <form action="{{ route('repairs.store') }}" method="POST" class="needs-validation" novalidate>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="customer_name" class="form-control" value="{{ old('customer_name', request('type')==='software' ? 'مجهول' : '') }}" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">اسم الجهاز</label>
                        <input type="text" name="device_name" class="form-control" value="{{ old('device_name', request('type')==='software' ? 'لاب توب' : '') }}" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">الموديل</label>
                        <input type="text" name="model" class="form-control" value="{{ old('model', request('type')==='software' ? 'لاب توب' : '') }}" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">رقم الجوال</label>
                        <input type="text" name="phone" class="form-control" value="{{ old('phone', request('type')==='software' ? '0' : '') }}" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">البريد الإلكتروني (اختياري — لإشعار "جاهز للاستلام")</label>
                        <input type="email" name="email" class="form-control" value="{{ old('email') }}">
                    </div>
                    <div class="col-md-12">
                        <label class="form-label">المشكلة / العطل</label>
                        <textarea name="issue" class="form-control" rows="3" required>{{ old('issue', request('type')==='software' ? 'تركيب ويندوز/برامج' : '') }}</textarea>
                    </div>
                    <div class="col-md-6">
