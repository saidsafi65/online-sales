                ...
                <form action="{{ route('mobile-shop.sales.store') }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="product_name" class="form-label" style="font-weight: 600; color: #1e293b;">اسم
                                المنتج</label>
                            <select class="form-select @error('product_name') is-invalid @enderror" id="product_name"
                                name="product_name" required>
                                <option value="">اختر المنتج</option>
                                @foreach (($products ?? collect())->keys() as $product)
                                    <option value="{{ $product }}"
                                        {{ old('product_name') == $product ? 'selected' : '' }}>
                                        {{ $product }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            <label for="product_type" class="form-label" style="font-weight: 600; color: #1e293b;">النوع /
                                الموديل</label>
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            <label for="quantity" class="form-label"
                                style="font-weight: 600; color: #1e293b;">الكمية</label>
                        ...
                        <div class="col-md-4 mb-3">
                            <label for="payment_method" class="form-label" style="font-weight: 600; color: #1e293b;">طريقة
                                الدفع</label>
                            <select class="form-select @error('payment_method') is-invalid @enderror" id="payment_method"
                                name="payment_method" required>
                                <option value="">اختر طريقة الدفع</option>
                                <option value="نقدي" {{ old('payment_method') === 'نقدي' ? 'selected' : '' }}>نقدي
                                </option>
                                <option value="تطبيق" {{ old('payment_method') === 'تطبيق' ? 'selected' : '' }}>تطبيق
                                </option>
                                <option value="مختلط" {{ old('payment_method') === 'مختلط' ? 'selected' : '' }}>مختلط
                                </option>
                            </select>
                    ...
                    <div class="row" id="payment_amounts">
                        <div class="col-md-6 mb-3" id="cash_amount_group">
                            <label for="cash_amount" class="form-label" style="font-weight: 600; color: #1e293b;">قيمة
                                التكلفة نقدي</label>
                            <input type="number" class="form-control @error('cash_amount') is-invalid @enderror"
                                id="cash_amount" name="cash_amount" step="0.01" value="{{ old('cash_amount', 0) }}"
                                required>
                            @error('cash_amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6 mb-3" id="bank_amount_group">
                            <label for="bank_amount" class="form-label" style="font-weight: 600; color: #1e293b;">قيمة
                                التكلفة بنكي</label>
                            <input type="number" class="form-control @error('bank_amount') is-invalid @enderror"
                                id="bank_amount" name="bank_amount" step="0.01" value="{{ old('bank_amount', 0) }}"
                                required>
                            @error('bank_amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="created_at" class="form-label" style="font-weight: 600; color: #1e293b;">تاريخ
                                المبيعة</label>
                            <input type="date" class="form-control @error('created_at') is-invalid @enderror"
                                id="created_at" name="created_at" value="{{ old('created_at', now()->toDateString()) }}">
                            <small class="text-muted">اتركه فارغاً لاستخدام تاريخ اليوم تلقائياً</small>
                            @error('created_at')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
