...
@section('content')
    <div class="container-fluid">
        ...
        <div class="card" style="border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
            <div class="card-body">
                <form action="{{ route('mobile-shop.sales.update', $sale) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('product_name') is-invalid @enderror" 
                                   id="product_name" name="product_name" value="{{ old('product_name', $sale->product_name) }}" required>
                            @error('product_name')
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('product_type') is-invalid @enderror" 
                                   id="product_type" name="product_type" value="{{ old('product_type', $sale->product_type) }}" required>
                            @error('product_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('payment_method') is-invalid @enderror" 
                                ...
                                <option value="نقدي" {{ old('payment_method', $sale->payment_method) === 'نقدي' ? 'selected' : '' }}>نقدي</option>
                                <option value="تطبيق" {{ old('payment_method', $sale->payment_method) === 'تطبيق' ? 'selected' : '' }}>تطبيق</option>
                                <option value="مختلط" {{ old('payment_method', $sale->payment_method) === 'مختلط' ? 'selected' : '' }}>مختلط</option>
                            </select>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('cash_amount') is-invalid @enderror" 
                                   id="cash_amount" name="cash_amount" step="0.01" value="{{ old('cash_amount', $sale->cash_amount ?? 0) }}" required>
                            @error('cash_amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('bank_amount') is-invalid @enderror" 
                                   id="bank_amount" name="bank_amount" step="0.01" value="{{ old('bank_amount', $sale->bank_amount ?? 0) }}" required>
                            @error('bank_amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
