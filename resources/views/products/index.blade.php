...
.sort-select {
    ...
    font-weight:600;cursor:pointer;transition:border-color var(--t);max-width:100%;
}
...
.wishlist-btn {
    position:absolute;top:.6rem;left:.6rem;z-index:5;width:32px;height:32px;border-radius:50%;
    background:rgba(255,255,255,.92);border:none;display:flex;align-items:center;justify-content:center;
    box-shadow:0 2px 8px rgba(0,0,0,.15);color:var(--brand-dark);font-size:.95rem;
}
.wishlist-btn.active { background:var(--brand-dark);color:#fff; }
.badge {
...
.btn-add-cart {
    ...
    width: 100%; padding: .55rem; border: none; border-radius: var(--r-sm);
    background: var(--brand); color: white; font-family: inherit; font-weight: 700;
    font-size: .85rem; cursor: pointer; text-decoration: none; transition: background var(--t);
}
.btn-add-cart:hover { background: var(--brand-dark); color: white; }
.btn-add-cart:disabled { opacity: .7; cursor: default; }
.btn-add-cart-guest { background: var(--slate-700); }
.btn-add-cart-guest:hover { background: var(--slate-900); }
...
#modal-box {
    ...
    width:100%;max-width:780px;height:88vh;max-height:88vh;overflow:hidden;
...
#modal-img-col { position:relative;background:var(--slate-100);height:100%;min-height:0;overflow:hidden; }
...
#modal-img-placeholder {
    ...
    font-size:5rem;color:var(--slate-300);background:linear-gradient(135deg,var(--slate-100) 0%,var(--brand-light) 100%);
}
#modal-oos-overlay {
...
#modal-info-col { height:100%;padding:1.85rem;display:flex;flex-direction:column;gap:.95rem;overflow-y:auto;min-height:0;min-width:0; }
...
#modal-name         { font-size:1.45rem;font-weight:900;color:var(--slate-900);margin:0;line-height:1.25;flex-shrink:0; }
...
#modal-discount-badge {
    ...
    border-radius:50px;padding:.45rem 1.1rem;font-weight:800;font-size:.88rem;text-align:center;flex-shrink:0;
}
#modal-close {
    ...
    <form method="GET" action="{{ route('products.index') }}" id="filterForm" autocomplete="off">
        ...
        <div class="shop-layout">
            ...
            <main class="main-col">
                ...
                <div class="toolbar">
                    @php
                        $activeFilterCount = collect(request()->only(['search','category','price_min','price_max','discount','in_stock']))
                            ->filter(fn($v) => $v !== null && $v !== '')->count();
                    @endphp
...
const CART_ADD_BASE_URL = "{{ url('/cart/add') }}";
...
function addToCart(productId, btnEl) {
    ...
    .then(res => {
        ...
        // تحديث عداد السلة بالهيدر فوراً بدون تحديث الصفحة
        if (typeof window.currentCartCount === 'number') {
            window.currentCartCount += 1;
            if (typeof updateCartBadge === 'function') {
                updateCartBadge(window.currentCartCount);
            }
        }
...
function openModal(id, name, category, price, discount, image, description, isOOS) {
    document.getElementById('modal-name').textContent     = name;
    document.getElementById('modal-category').textContent = category;
    document.getElementById('modal-desc').textContent     = description || 'لا يوجد وصف لهذا المنتج.';

    currentModalProductId = id;
    const modalBtn = document.getElementById('modal-add-cart-btn');
    if (modalBtn) {
        modalBtn.disabled = isOOS;
