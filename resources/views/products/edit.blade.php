...
<div class="mb-4">
    <h3 class="form-section-title">
        ...
        ربط بعنصر من الكتالوج (اختياري)
    </h3>
    <p style="font-size:.88rem;color:#64748b;margin:-.5rem 0 .75rem;">
        إذا ربطت المنتج بعنصر من الكتالوج، حالة "نفذت الكمية" رح تتحدث تلقائيًا حسب كمية هذا العنصر —
        بدون ما تضطر تعدّلها يدويًا كل مرة.
    </p>
    <select name="catalog_item_id" id="catalogItemSelect" class="form-control" onchange="onCatalogLinkChange()">
        <option value="">بدون ربط (تحديد الحالة يدويًا)</option>
        @foreach($catalogItems as $item)
            <option value="{{ $item->id }}"
                {{ (old('catalog_item_id', $product->catalog_item_id) == $item->id) ? 'selected' : '' }}>
...
<!-- حالة المخزون -->
<div class="mb-4" id="manualStockSection">
    <h3 class="form-section-title">
        <span class="section-icon">
    ...
    <div style="display:none; padding: 1rem 1.5rem; background: #eff6ff; border: 2px solid #bfdbfe;
                border-radius: 16px; color: #1e40af; font-weight: 600; align-items:center; gap:.6rem;"
         id="linkedStockNotice">
        <i class="fas fa-info-circle"></i>
        <span>حالة التوفر بتتحدد تلقائيًا حسب كمية عنصر الكتالوج المرتبط — الزر أعلاه معطّل حاليًا.</span>
    </div>
</div>
                <!-- معلومات المنتج -->
                <div class="mb-4">
...
function onCatalogLinkChange() {
    ...
    const stockNotice = document.getElementById('linkedStockNotice');
    const quantityInput = document.getElementById('quantity');
    const quantityNotice = document.getElementById('quantityLinkedNotice');
    const isLinked = select.value !== '';

    stockBtn.disabled = isLinked;
    stockBtn.style.opacity = isLinked ? '.5' : '1';
    stockBtn.style.cursor = isLinked ? 'not-allowed' : 'pointer';
