...
@push('mobile-filter-bar')
    @php
        $__mfbActiveCount = collect(request()->only(['search','brand','price_min','price_max','in_stock']))
    ...
    <div class="mobile-filter-chip-bar">
        ...
        <button type="button" class="filter-chip-btn {{ !request('brand') ? 'active' : '' }}" onclick="selectBrand('')">
            كل الماركات
        </button>
        @foreach($brands as $brand => $cnt)
            <button type="button" class="filter-chip-btn {{ request('brand') === $brand ? 'active' : '' }}" onclick="selectBrand('{{ $brand }}')">
                {{ $brand }}
            </button>
        @endforeach
    </div>
