...
class RepairsController extends Controller
{
    ...
    public function update(Request $request, Repair $repair): RedirectResponse
    {
        ...
        try {
            ...
            // نلتقط هذا قبل التحديث عشان نعرف إذا هالتحديث هو أول مرة صار فيها الجهاز "جاهز"
            $justBecameReady = ! $repair->delivery_date && $request->filled('delivery_date');
            ...
            if ($justBecameReady && $repair->email) {
                try {
                    \Illuminate\Support\Facades\Notification::route('mail', $repair->email)
                        ->notify(new \App\Notifications\RepairReadyNotification($repair));
                } catch (\Throwable $e) {
                    \Illuminate\Support\Facades\Log::warning('RepairReadyNotification failed', ['repair_id' => $repair->id, 'error' => $e->getMessage()]);
                }
            }
