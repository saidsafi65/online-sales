...
class SalesController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            // تنبيه غير ملزم لو المبلغ المسجّل أقل من سعر الكتالوج × الكمية (ممكن يكون
            // خصم أو تنزيلة مقصودة — بس منبّه لحالة الغلط أو النسيان).
            $priceWarning = null;
            $expectedTotal = (float) ($catalogItem->sale_price ?? 0) * $requestedQuantity;
            $enteredTotal = (float) $request->cash_amount + (float) $request->app_amount;
            if ($expectedTotal > 0 && $enteredTotal < $expectedTotal) {
                $priceWarning = 'تنبيه: المبلغ المسجّل ('.number_format($enteredTotal, 2).') أقل من السعر المعتاد لهالمنتج ('.number_format($expectedTotal, 2).'). تأكد إنه مقصود.';
            }
    ...
    public function returnSale(int $saleId): JsonResponse
    {
        ...
        try {
            ...
            // نحفظ المبلغ الأصلي بالملاحظات (سجل تدقيق) قبل ما نصفّره — المبلغ رجع فعليًا
            // للزبون، فلازم ما يضل محسوب كإيراد، وبما إنه فلوس خرجت فعليًا من الدرج، لازم
            // يبقى أثر واضح إنه في مبلغ اترجع مش بس "الكمية اترجعت".
            $refundedCash = (float) $sale->cash_amount;
            $refundedApp  = (float) $sale->app_amount;
            ...
            $sale->update([
                ...
                'notes' => ($sale->notes ? $sale->notes.' - ' : '')
                    .'تم الإرجاع في '.now()->format('Y-m-d H:i:s')
                    ." (مبلغ مسترجع: نقدي {$refundedCash} + تطبيق {$refundedApp})",
            ]);
